...
// Procura por usuários com o mesmo usuário e senha
        ...
        $query = $this->select("and {$binary} `{$this->campos['login']}` = '{$usuario}' AND {$binary} `{$this->campos['senha']}` = '{$senha}'");
...
// Consulta os dados
                $sql = "SELECT {$dados} FROM dbservweb.`{$this->tabelaUsuarios}` WHERE {$binary} `{$this->campos['login']}` = '{$usuario}'";
...
// Monta o valor do cookie
                $valor = join('#', array($_SESSION[$this->prefixoChaves . 'usuario_0'], $_SERVER['REMOTE_ADDR'], $_SERVER['HTTP_USER_AGENT']));
